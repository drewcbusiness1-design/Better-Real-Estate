...
function routeOrder({ order, item, supplier, buyer, shipping, crypto }) {
  return {
    ...
    supplierKind: supplier ? supplier.kind : null,
    supplierSku: item.supplierSku || null,
    cjPid: item.cjPid || null,
    cjVid: item.cjVid || item.supplierSku || null,
    cjVariantSku: item.cjVariantSku || null,
    cjFromCountryCode: item.cjFromCountryCode || 'CN',
    title: item.title,
    qty: 1,
    productCostCents: item.cost || 0,
    shippingCostCents: Number(order.shippingCostCents || 0) || 0,
    costCents: (item.cost || 0) + (Number(order.shippingCostCents || 0) || 0),
    chargedCents: order.price || item.price,
    cjLogisticName: order.cjLogisticName || null,
    cjQuotedDays: order.cjQuotedDays || null,
