...
function normalizeVariant(v, product) {
  ...
  return {
    ...
    unit: v.variantUnit || product?.productUnit || '',
    ...
    lengthMm: Number(v.variantLength || 0) || null,
    widthMm: Number(v.variantWidth || 0) || null,
    heightMm: Number(v.variantHeight || 0) || null,
    volumeMm3: Number(v.variantVolume || 0) || null,
...
function normalizeProduct(p, includeVariants = false) {
  const product = {
    ...
    suggestedPrice: p.suggestSellPrice || p.suggestedPrice || null,
    categoryId: p.categoryId || '',
    categoryName: p.categoryName || p.threeCategoryName || p.twoCategoryName || p.oneCategoryName || p.category || '',
    description: cleanHtml(p.description || '').slice(0, 3000),
    weight: Number(p.productWeight || 0) || null,
    packingWeight: Number(p.packingWeight || 0) || null,
    unit: p.productUnit || '',
